// MangaVerse Backend (History Fixed)
...
// ... (register, login, seed, and other manga routes remain the same) ...
...
// POST /api/mangas/seed - Seed database with sample manga
app.post('/api/mangas/seed', async (req, res) => {
    try {
        await Manga.deleteMany({}); // Clear existing data for a clean seed
        const sampleMangas = [
            { title: "Attack on Titan", author: "Hajime Isayama", chapters: 139, rating: 4.8, genre: "Action, Fantasy", cover: "https://placehold.co/240x340/a32a28/ffffff?text=Attack+on+Titan" },
            { title: "Bleach", author: "Tite Kubo", chapters: 686, rating: 4.5, genre: "Shonen", cover: "https://placehold.co/240x340/1a1a1a/ffffff?text=Bleach" },
            { title: "One Piece", author: "Eiichiro Oda", chapters: 1100, rating: 4.9, genre: "Adventure", cover: "https://placehold.co/240x340/0077c2/ffffff?text=One+Piece" },
            { title: "Naruto", author: "Masashi Kishimoto", chapters: 700, rating: 4.7, genre: "Action, Ninja", cover: "https://placehold.co/240x340/ff7f00/ffffff?text=Naruto" },
            { title: "Jujutsu Kaisen", author: "Gege Akutami", chapters: 230, rating: 4.9, genre: "Supernatural", cover: "https://placehold.co/240x340/7b2cbf/ffffff?text=Jujutsu+Kaisen" }
        ];
...
// GET /api/mangas - Get all mangas with search and sort
...
// GET /api/mangas/:id - Get a single manga's details
...
// POST /api/mangas/:id/reviews - Add a review
...
// --- HISTORY ROUTES (CORRECTED) ---

// POST /api/history - Update a user's reading history
app.post('/api/history', verifyToken, async (req, res) => {
  try {
    ...
    // Add to their history and save the document
    user.history.push({ mangaTitle, chaptersRead, date: new Date() });
    await user.save();
    
    // **FIX:** Return the updated history array
...
// GET /api/history - Get a user's reading history
app.get('/api/history', verifyToken, async (req, res) => {
  try {
    // **FIX:** Simply find the user and return the history property
    const user = await User.findById(req.user.id);
    if (!user) return res.status(404).json({ message: 'User not found.' });
